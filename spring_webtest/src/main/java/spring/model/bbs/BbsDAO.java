...
public class BbsDAO {
  ...
  public boolean createReply(BbsDTO dto) {
    ...
    try {
      ...
      pstmt.setInt(5, dto.getGrpno());// �쁾遺�紐⑥쓽grpno
      pstmt.setInt(6, dto.getIndent() + 1);// �쁾遺�紐⑥쓽indent+1
      pstmt.setInt(7, dto.getAnsnum() + 1);// �쁾遺�紐⑥쓽ansnum+1
  ...
  public boolean update(BbsDTO dto) {
    ...
    try {
      ...
      pstmt.setInt(++i, dto.getBbsno());//if문이 있느냐 없느냐에 따라 순번이 달라짐
...
//  }
  ...
  public boolean create(BbsDTO dto) {
    ...
    StringBuffer sql = new StringBuffer();
    sql.append(" INSERT INTO bbs(bbsno, wname, title, ");
    sql.append(" content, passwd, wdate, grpno, filename, filesize)   ");//추가
    sql.append(" VALUES( ");
    sql.append(" (SELECT NVL(MAX(bbsno), 0)+1 FROM bbs),");
    sql.append(" ?, ?, ?, ?, sysdate,  ");
    sql.append(" (SELECT NVL(MAX(grpno), 0)+1 FROM bbs),?,? )");//추가
    ...
    try {
      ...
      pstmt.setString(3, dto.getContent());
      pstmt.setString(4, dto.getPasswd());
      pstmt.setString(5, dto.getFilename());//추가
      pstmt.setInt(6, dto.getFilesize());//추가
